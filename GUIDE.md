...
pick which agent should attempt it (Claude, Gemini, or a scripted mock for
testing), and CodeProof:

1. Clones the repo in an isolated Docker sandbox — never on your machine
2. Has the agent investigate the repo and try to **reproduce the bug for
   real** before trusting that it exists
3. Lets the agent propose and apply a fix, in that same sandbox
4. Re-runs the reproduction and the existing test suite to check the fix
   actually holds
5. Shows you the full evidence trail — every command run, every output,
...
3. Pick an agent:
   ...
   - **mock** — a scripted agent with one hardcoded fixture bug, no API key
     needed. Good for confirming the pipeline itself works, not for
     evaluating a real issue (it doesn't understand any other repo).
4. Click **Start Evaluation** and watch it move through PENDING → RUNNING →
   a final verdict. A live agent run can take several minutes — it's
   genuinely working through investigate → reproduce → fix → verify, not
   stuck.
5. On the result page, review the reproduction, the diff, the test results,
   the full evidence timeline, and the agent's raw trajectory. Then record
   your own call: Approve / Request Revision / Reject / Abstain.
...
| Verdict | Meaning |
|---|---|
| **PASS** | The agent reproduced the bug, its patch made the reproduction pass, and the existing test suite still passes. This is the strongest signal CodeProof currently gives — but see [Limitations](#limitations) below for what it does *not* yet check. |
| **FAIL** | The agent proposed a patch, but either the bug still reproduces after it, or existing tests broke. The patch did not actually fix the problem — or broke something else. |
...
- **No adversarial/Skeptic testing yet.** PASS today means "the one
  reproduction the agent wrote now passes, and the pre-existing test suite
  still passes" — not that the fix is robust to edge cases nobody wrote a
  test for. That's the planned next layer.
- **The mock agent only understands one fixture repo.** Don't expect it to
  evaluate a real, arbitrary issue — use a live agent for that.
- **Live agent behavior is non-deterministic.** The same issue can get a
  different verdict on different runs. A Reproducibility/Replay feature
  (run N times, compare) is planned but not built yet.
- **Free-tier LLM quotas are tight.** Gemini's free tier caps out at a
  ...
  meaningful chunk of that on a real multi-file repo.
