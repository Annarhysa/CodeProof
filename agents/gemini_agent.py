...
class GeminiCodingAgent(CodingAgent):
    ...
    def reproduce_issue(self) -> ReproductionResult:
        prompt = (
            ...
            "trying to execute code, since nothing will run otherwise. Use run_command to "
    ...
    def _generate_content_with_retry(self, tool: types.Tool) -> types.GenerateContentResponse:
        """The Gemini free tier's per-minute quota is tight (5 req/min on
        some models) and easy to hit mid-agentic-loop; retry with the
        server-suggested delay rather than let the whole evaluation crash."""
